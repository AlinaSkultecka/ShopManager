...
namespace ShopManager.Helpers
{
    public class ShopHelper
    {
        ...
        public static void ShowProductInfo(List<ProductProperties> products)
        {
            Console.Clear();
            Console.WriteLine("PRODUCT LIST");

            // Group by category
            var grouped = products.GroupBy(p => p.ProductCategory);
            ...
            foreach (var group in grouped)
            {
                ...
                foreach (var product in group)
                {
                    ...
                    Console.WriteLine(
                        $"ID: {product.Id} | Name: {product.Name} | Price: {product.Price.ToString("C", swedish)} | Stock: {product.Stock}");
                }
            }
        }
        ...
        public static async Task AddToShoppingCart(CustomerProperties user, MongoDbService db)
        {
            while (true)
            {
                // 1. Load products from MongoDB each time (so stock is up-to-date)
                var allProducts = await db.GetAllProductsAsync();
                ...
                // 2. Find product FROM DB LIST 
                ProductProperties? product = allProducts.FirstOrDefault(p => p.Id == productId);
        ...
        public static async Task ViewShoppingCart(CustomerProperties user, MongoDbService db)
        {
            //Console.Clear();
            CultureInfo swedish = new CultureInfo("sv-SE");
            ...
            while (running)
            {
                ...
                {
                    case "1":
                        await RemoveItemFromCart(user, db);
        ...
        private static async Task RemoveItemFromCart(CustomerProperties user, MongoDbService db)
        {
            ...
            if (itemToRemove != null)
            {
                ...
                if (qtyToRemove >= itemToRemove.Quantity)
                {
                    ...
                    // Update in DB
                    await db.UpdateProductAsync(itemToRemove.Product);
                ...
                else
                {
                    ...
                    Console.WriteLine(
                        $"{qtyToRemove} of {itemToRemove.Product.Name} removed from your cart. Remaining: {itemToRemove.Quantity}");
                }
            }
            else
            {
